...
export const useAuthHook = () => {
  ...
  const setSession = (expires: string) => {
    ...
    sessionStorage.setItem('sellerLoginTime', expirationTime.toISOString());
  };

  const clearSession = () => {
    sessionStorage.removeItem('sellerLoginTime');
    clearInterval(logoutTimer);
  };

  const getSessionExpirationTime = () => {
    return sessionStorage.getItem('sellerLoginTime');
  };
  ...
  const logoutUser = () => {
    ...
    localStorage.removeItem('persist:LocalVendaSeller');
